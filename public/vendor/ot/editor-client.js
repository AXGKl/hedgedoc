...
ot.EditorClient = (function () {
  ...
  OtherMeta.fromJSON = function (obj) {
    return new OtherMeta(obj.clientId, obj.selection && Selection.fromJSON(obj.selection))
  }
  ...
  function OtherClient(id, listEl, editorAdapter, name, color, selection) {
    ...
    if (!color) this.setColor(name ? hueFromName(name) : Math.random())
    else this.setForceColor(color)
    if (selection) {
  ...
  function EditorClient(revision, clients, serverAdapter, editorAdapter) {
    ...
    this.serverAdapter.registerCallbacks({
      ...
      set_color: function (clientId, color) {
        self.getClientObject(clientId).setForceColor(color)
      },
      ack: function (revision) {
      ...
      selection: function (clientId, selection) {
        if (selection) {
          self
            ...
            .updateSelection(self.transformSelection(Selection.fromJSON(selection)))
        } else {
