  ...
  new SamlStrategy(
    {
      ...
      privateKey:
        config.saml.clientCert === undefined
          ? undefined
          : (function () {
              try {
    ...
    function (user, done) {
      ...
      if (config.saml.externalGroups && config.saml.groupAttribute) {
        const externalGroups = intersection(
          config.saml.externalGroups,
          user[config.saml.groupAttribute]
        )
      ...
      if (config.saml.requiredGroups && config.saml.groupAttribute) {
        if (
          intersection(config.saml.requiredGroups, user[config.saml.groupAttribute])
            .length === 0
        ) {
