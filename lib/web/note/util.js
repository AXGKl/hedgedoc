...
exports.newNote = async function (req, res, body) {
  ...
  if (noteId) {
    if (
      config.allowFreeURL &&
      !config.forbiddenNoteIDs.includes(noteId) &&
      (!config.requireFreeURLAuthentication || req.isAuthenticated())
    ) {
      req.alias = noteId
    } else {
  ...
  models.Note.create({
    ...
    .then(function (note) {
      return res.redirect(
        config.serverURL +
          '/' +
          (note.alias ? note.alias : models.Note.encodeNoteId(note.id))
      )
    })
    .catch(function (err) {
...
exports.getPublishData = function (req, res, note, callback) {
  ...
  const data = {
    title,
    description:
      meta.description || (markdown ? models.Note.generateDescription(markdown) : null),
    lang: meta.lang || null,
    ...
    lastchangeuserprofile: note.lastchangeuser
