...
if (!['strict', 'lax', 'none'].includes(config.cookiePolicy)) {
  logger.error(
    'Cookie SameSite policy %s does not exist. Falling back to lax. Available values are: strict, lax, none.',
    config.cookiePolicy
  )
...
if (config.gitlab && config.gitlab.version !== 'v4' && config.gitlab.version !== 'v3') {
  logger.warn(
    'config.json contains wrong version (' +
...
config.isGitlabSnippetsEnable =
  (!config.gitlab.scope || config.gitlab.scope === 'api') && config.isGitLabEnable
...
for (let i = keys.length; i--; ) {
  ...
  if (
    ...
    logger.warn(
      'config.json contains deprecated lowercase setting for ' +
        keys[i] +
        '. Please change your config.json file to replace ' +
        lowercaseKey +
        ' with ' +
        keys[i]
    )
...
if (Object.keys(process.env).toString().indexOf('HMD_') !== -1) {
  logger.warn(
    'Using legacy HMD prefix for environment variables. Please change your variables in future. For details see: https://github.com/hedgedoc/hedgedoc/blob/master/docs/configuration.md'
  )
}
...
// GK: The checks had been totally disabled, in order to allow any upload. We are in company scenario.
// Disabled in: "lib/web/imageRouter/index.js"
switch (config.imageUploadType) {
  case 'imgur':
    config.allowedUploadMimeTypes = ['image/jpeg', 'image/png', 'image/gif']
    break
  default:
