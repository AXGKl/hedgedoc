...
function isReady() {
  return (
    ...
    connectionSocketQueue.length === 0 &&
    !isConnectionBusy &&
    disconnectSocketQueue.length === 0 &&
    !isDisconnectBusy
  )
}
  ...
  if (!notes[noteId]) {
    ...
    models.Note.findOne({
      ...
      .then(function (note) {
        ...
        const lastchangeuserprofile = note.lastchangeuser
          ? models.User.getProfile(note.lastchangeuser)
          : null

        const body = note.content
        const createtime = note.createdAt
        const updatetime = note.lastchangeAt
        const server = new ot.EditorSocketIOServer(
          body,
          [],
          noteId,
          ifMayEdit,
...
function operationCallback(socket, operation) {
  ...
  setImmediate(function () {
    note.authorship = models.Note.updateAuthorshipByOperation(
      operation,
      userId,
      note.authorship
    )
...
function connection(socket) {
  ...
  socket.on('user status', function (data) {
    ...
    if (!noteId || !notes[noteId] || !user) return
    logger.debug(
      `SERVER received [${noteId}] user status from [${socket.id}]: ${JSON.stringify(data)}`
    )
    if (data) {
      user.idle = data.idle
      user.type = data.type
