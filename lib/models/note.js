...
module.exports = function (sequelize, DataTypes) {
                ...
                if (Note.checkFileExist(filePath)) {
                  ...
                  if (fsModifiedTime.isAfter(dbModifiedTime) && note.content !== body) {
                    note
                      ...
                      .then(function (note) {
                        sequelize.models.Revision.saveNoteRevision(
                          note,
                          function (err, revision) {
                            if (err) return _callback(err, null)
                            // update authorship on after making revision of docs
                            const patch = dmp.patch_fromText(revision.patch)
                            const operations = Note.transformPatchToOperations(
                              patch,
                              contentLength
                            )
                            let authorship = note.authorship
                            for (let i = 0; i < operations.length; i++) {
                              authorship = Note.updateAuthorshipByOperation(
                                operations[i],
                                null,
                                authorship
                              )
        ...
        parseNoteIdByLZString: function (_callback) {
          ...
          try {
            ...
            if (err.message === "Cannot read property 'charAt' of undefined") {
              logger.warning(
                'Looks like we can not decode "' +
                  noteId +
                  '" with LZString. Can be ignored.'
              )
            } else {
              logger.error(err)
            }
  ...
  Note.parseMeta = function (meta) {
    ...
    if (meta) {
      if (
        meta.title &&
        (typeof meta.title === 'string' || typeof meta.title === 'number')
      ) {
        _meta.title = meta.title
      }
      if (
        meta.description &&
        (typeof meta.description === 'string' || typeof meta.description === 'number')
      ) {
        _meta.description = meta.description
      }
      if (
        meta.robots &&
        (typeof meta.robots === 'string' || typeof meta.robots === 'number')
      ) {
        _meta.robots = meta.robots
      }
      if (meta.lang && typeof meta.lang === 'string') {
        _meta.lang = meta.lang
      }
      ...
      if (
        meta.disqus &&
        (typeof meta.disqus === 'string' || typeof meta.disqus === 'number')
      ) {
  ...
  Note.parseOpengraph = function (meta, title) {
    ...
    if (
      !(
        _ogdata.title &&
        (typeof _ogdata.title === 'string' || typeof _ogdata.title === 'number')
      )
    ) {
      _ogdata.title = title
    }
    if (
      !(
        _ogdata.description &&
        (typeof _ogdata.description === 'string' || typeof _ogdata.description === 'number')
      )
    ) {
  ...
  Note.updateAuthorshipByOperation = function (operation, userId, authorships) {
    ...
    for (let i = 0; i < operation.length; i++) {
      ...
      if (ot.TextOperation.isRetain(op)) {
        ...
        if (authorships.length <= 0)
          authorships.push([userId, opStart, opEnd, timestamp, timestamp])
        else {
          for (let j = 0; j < authorships.length; j++) {
            ...
            if (!inserted) {
              ...
              if (
                (nextAuthorship !== -1 && nextAuthorship[1] >= opEnd) ||
                j >= authorships.length - 1
              ) {
    ...
    for (let j = 0; j < authorships.length; j++) {
      ...
      for (let k = j + 1; k < authorships.length; k++) {
        ...
        if (
          nextAuthorship &&
          authorship[0] === nextAuthorship[0] &&
          authorship[2] === nextAuthorship[1]
        ) {
